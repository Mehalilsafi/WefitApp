...
export default function Featured() {
  ...
  const ulClasses = `flex flex-col gap-2 ${
    window.innerWidth > 768 ? "h-16" : "h-auto"
  } rounded p-5 mt-6 md:flex-row`;

  return (
    <div>
      <div
        className="flex justify-center border-2 border-[#A2FF86] p-2 w-36 bg-[#F9F9F9] rounded-xl ml-2.5 md:hidden"
        onClick={toggleFeatured}
      >
        <FontAwesomeIcon icon={faDumbbell} />
      </div>
      {(isFeaturdOpen || window.innerWidth > 768) && (
        <ul className={ulClasses}>{Featured}</ul>
      )}
    </div>
  );
}
